  ...
  function(collection, hub = NULL, site = NULL, date = NULL, newData = FALSE, viewer = "dialog") 
  {
    ...
    server = function(input, output, session) {
      ...
      observeEvent(input$done, {
        # Create a backup of the data being replaced:
        save_records(collection, hub, site, date)
        # Remove old data:
        if(newData == FALSE){
          if(!is.null(hub) & is.null(site)){
            siteVector <-
...
# Here are all of the names of the tables: 
...
# Start by installing editData package: install.packages('editData')
# This puts the "edit data" tool in your "Addins" menu

# Get data for a given table (collection), site, and date:
...
# Modify example data (local copy):

fixedData <-
  editData(exampleData)

# Replace data in database:

replace_siteDate(
  replacementDf = fixedData, 
  collection = 'captures', 
  site = 'DURBLAUNC1',
  date = '2017-06-15'
  )
...
# Get blank data records:

blankData <- 
  getMongo_blank(collection = 'partRs')
...
insert_newRecord(newData, 'partRs')
