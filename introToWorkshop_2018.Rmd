...
<li><div class = "roundBorderEx">Blue boxes provide summary information on keyboard shortcuts, operators, functions, vocabulary, and best management practices associated with a lesson. Pay careful attention to the material in blue boxes!</div></li>
</ul>
...
<p> The  <span class="tt">console panel<span class="ttText">A panel of R Studio used to execute code and writing/running code that users do not intend to replicate.</span></span> provides the command line interface to the R program itself. When a line of <span class="tt">code<span class="ttText">Any text written in the source or console panel.</span></span> is run from the command prompt (`>`) on the console, the interface sends the code to R and R which evaluates it and returns any potential output. When you ran the above sections of code from the  <span class="tt">source panel     <span class="ttText">A panel of R Studio for writing, editing, and running scripts.</span>   </span>, you may have noticed that the input and output was appeared in the console panel.This highlights a key component of the internal R Studio workflow: code written in the  source panel is sent to the console panel (CTRL+Enter), the console panel sends the code to R for evaluation, and R prints the results in the console panel (if applicable).</p>
...
<li><b>Camel case</b>: A convention of writing in which multiple words are joined without spaces and words following the initial word are capitalized</li>
